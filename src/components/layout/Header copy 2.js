...
const Navbar = styled(NavbarBase)`
    ...
    transition-timing-function: ${props => (props.show ? "ease-in" : "ease-out")};
...
const Header = ({history,show}) => {
    ...
    return(
        <>
        <Navbar show={(history.location.pathname !== '/' &&  history.location.pathname !== '/projects') ? show  : NavbarBase} className="container-fluid p32-x lg-p56-x" id="header">
